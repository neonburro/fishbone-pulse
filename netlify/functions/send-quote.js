...
function quoteHtml(q, link) {
  ...
  const items = (q.items || []).map((it) => {
    ...
    return `<tr><td style="padding:10px 0;vertical-align:top"><div style="font-family:${FONT};font-size:14px;font-weight:600;color:${BONE}">${esc(it.product_name)}${it.variant_label ? `, ${esc(it.variant_label)}` : ''}</div><div style="font-family:${MONO};font-size:11px;color:${MUTE};margin-top:3px">${esc([it.quantity ? `${it.quantity} pcs` : null, where, sizes].filter(Boolean).join(', '))}</div>${it.notes ? `<div style="font-family:${FONT};font-size:13px;color:${MUTE};margin-top:4px">${esc(it.notes)}</div>` : ''}</td><td align="right" style="padding:10px 0;vertical-align:top;font-family:${MONO};font-size:13px;color:${BONE};white-space:nowrap">${money(it.line_total)}</td></tr>`
  }).join('')
