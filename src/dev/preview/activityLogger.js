...
export async function logActivity(action, entityType, entityId, entityName, details = {}) {
  state.activity.unshift({ id: `act-${Date.now()}`, user_id: fx.previewUser.id, user_email: fx.previewUser.email, user_name: 'Tyler Reagan', action, entity_type: entityType, entity_id: entityId, entity_name: entityName, details, created_at: new Date().toISOString() })
}
